...
export default auth((req): any => {
  ...
  const isPosterOAuth = pathname.startsWith("/api/poster"); // allow Poster OAuth endpoints
  const hasPosterCodeOnHome =
    pathname === "/" &&
    (nextUrl.searchParams.has("code") ||
      nextUrl.searchParams.has("account") ||
      nextUrl.searchParams.has("error"));

  // public routes check (exact match)
  const isPublicRoute = publicRoutes.includes(pathname);

  // Always allow: next-auth API, Poster OAuth endpoints, public routes, and home with ?code
  if (isApiAuthRoute || isPosterOAuth || isPublicRoute || hasPosterCodeOnHome) {
    return null;
  }
